...
#define KEY0 0xcb463f34a1876a72
#define KEY1 0xefef5bbf5d807619

#define INPUT_DIFF 0x00a000a0000a0a00
#define COND_MASK 0xff00ff0fff0fffff
#define COND 0x00000000000a0a00
...
static inline __device__ uint64_t round(const uint64_t old_st, const uint64_t tk0, const uint64_t tk1, const uint64_t tk2, const uint64_t tk3, const int r) {


	const uint64_t S[16] = {0xc, 0xa, 0xd, 0x3, 0xe, 0xb, 0xf, 0x7, 0x8, 0x9, 0x1, 0x5, 0x0, 0x2, 0x4, 0x6};
	const uint64_t RC[32] = 
		{0x110000, 0x480000, 0x240000, 0x520000, 0x690000, 0x7c0000, 0x360000, 0x1b0000, 
...
__global__ void test_correct_pairs(const uint64_t *d_init_values, uint64_t *d_correct_num) {
	

	// recieve input data
	uint64_t cur_idx = blockDim.x * blockIdx.x + threadIdx.x;
	uint64_t p0_lfsr = d_init_values[cur_idx * 2];
	uint64_t tweak_lfsr = d_init_values[cur_idx * 2 + 1];
	uint64_t counter_for_correct_pairs = 0;

	// consts
	const uint64_t one = 1;
	const uint64_t mask16 = 0xf;

	

 	const uint64_t key0 = KEY0;
    const uint64_t key1 = KEY1;
    ...
    const uint64_t tweak_mask = (0xffffffffffffffff ^ LEFTROTBY4(mask16, 3) ^ LEFTROTBY4(mask16, 9) ^ LEFTROTBY4(mask16, 0) ^ LEFTROTBY4(mask16, 13) ^ LEFTROTBY4(mask16, 7) ^ LEFTROTBY4(mask16, 10));
    // prepare values in tweak to be set.
	const uint64_t key1_setto_tweak = (LEFTROTBY4(GETBY4(key1, 9), 3) | LEFTROTBY4(GETBY4(key1, 9), 9) | LEFTROTBY4(GETBY4(key1, 13), 0) | LEFTROTBY4(GETBY4(key1, 13), 13) | LEFTROTBY4(GETBY4(key1, 10), 7) | LEFTROTBY4(GETBY4(key1, 10), 10));


	const uint64_t diff = INPUT_DIFF;
    const uint64_t condition_mask = COND_MASK;
    const uint64_t condition = COND;
	
	uint64_t p0, p1, tweak;
	uint64_t tk0, tk1, tk2, tk3, qt;


	const uint64_t data_num = (one << ENUM_NUM);


	// main loop in each device
	for (uint64_t data_i = 0; data_i < data_num; data_i++) {
        ...
        // test correct pairs
        if (((p0 ^ p1) & condition_mask) == condition) {
...
int main(int argc, char** argv) {
	...
	if (argc >= 2) {
		...
		if (argc == 3) {
			...
			srand(time(0));
			printf("finish setting device %d with random seed\n", atoi(argv[1]));
		}
	} else {
		cudaSetDevice(0);
		srand(time(0));
		printf("finish setting default device with random seed\n");
	}
	...
	uint64_t *d_correct_num = NULL;
	err = cudaMalloc((void **) &d_correct_num, thr_num * sizeof(uint64_t));
	if (err != cudaSuccess) {
	...
	printf("Round : %d\n", ROUND_NUM);
	printf("key0 : %lx, key1 : %lx\n", KEY0, KEY1);
	printf("input diff : %lx\n", INPUT_DIFF);
	printf("output form : %lx\n", COND_MASK);
	...
	uint64_t h_correct_num[thr_num];

	err = cudaMemcpy(h_correct_num, d_correct_num, thr_num * sizeof(uint64_t), cudaMemcpyDeviceToHost);
	if (err != cudaSuccess) {
